    ...
    <div class="container" id="container_column">
        <div class='titulo'>
            ...
            foreach ($tipo as $data) :
                if ($data['id_tipo'] == $id_tipo) {
                    echo "<h1 class=''>" . $data['nome_tipo'] . "</h1>";
                }
            endforeach;
            ?>
        </div>
        <div class="locales">
            <?php
            if (isset($comercio)) :
                foreach ($comercio as $data) :
                    if ($data['id_comercio'] == $id_tipo && count($data['id_comercio']) >= 1) {
                        // print_r($data);
            ?>
...
<script>
    $(function() {
        function parca(id) {
            let pc = document.querySelectorAll(`#parceiro${parceiro}`);
            let id = document.querySelectorAll(`#parca${id}`);
